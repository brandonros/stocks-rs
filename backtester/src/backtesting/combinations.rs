...
pub fn build_indicator_setting_combinations(mode: &str, strategy: &Strategy) -> Vec<StrategyIndicatorSettings> {
  if mode == "cartesian" {
...
pub fn build_backtest_setting_combinations(mode: &str, warmed_up_index: usize, slippage_percentage: f64) -> Vec<BacktestSettings> {
  if mode == "cartesian" {
    let profit_limit_percentages = build_decimal_range(dec!(0.001), dec!(0.01), dec!(0.0005));
