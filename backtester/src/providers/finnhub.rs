...
use crate::{
  http_client,
  structs::{Candle, FinnhubStockCandlesResponse},
};
...
pub async fn get_candles(symbol: &str, resolution: &str, from: DateTime<Tz>, to: DateTime<Tz>) -> Result<Vec<Candle>, String> {
  ...
  let result = http_client::http_request_json::<FinnhubStockCandlesResponse>(&http_client, "GET", &request_url, &request_headers, &None).await;
  if result.is_err() {
