...
pub fn get_extended_market_session_start_and_end(eastern_now: &DateTime<Tz>) -> (DateTime<Tz>, DateTime<Tz>) {
  ...
  let regular_market_start = Eastern.with_ymd_and_hms(year, month, day, 7, 0, 0).unwrap(); // 7:00:00am
  let regular_market_end = Eastern.with_ymd_and_hms(year, month, day, 15, 59, 59).unwrap(); // 3:59:59pm // TODO: 8pm?
